...
def load_hdf5(dataset_dir, dataset_name):
    ...
    with h5py.File(dataset_path, 'r') as root:
        ...
        if 'master_positions' in root:
            ...
            action_puppet = root['send_position_robots'][()] if 'send_position_robots' in root else None
...
def main(args):
    episode_idx = 30
    dataset_dir = "../tos_app_data/20250630_182048"
    # find most recent directory and add to dataset_dir
    ...
    # Visualize joints
    plot_path = os.path.join(dataset_dir, dataset_name + '_joint_pos.png')
...
def visualize_joints(action_master, action_puppet, joint_states, action, plot_path=None, ylim=None, label_overwrite=None):
    ...
    for dim_idx in range(num_dim):
        ax = axs[dim_idx]
        ax.plot(action_puppet[:, dim_idx], label='action puppet')
        ax.legend()

    for dim_idx in range(num_dim):
        ax = axs[dim_idx]
        ax.plot(action_master[:, dim_idx], label='action master')
        ax.legend()
