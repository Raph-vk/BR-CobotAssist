...
class RobotController:
    ...
    def _init_interface_states(self):
        ...
        if self.control_loop_language == "python":
            ...
            size = int(self.record_duration / self.control_dt)
            # round size up to nearest next factor of 2
            size = 2 ** (size - 1).bit_length()
            self.initialize_shared_memory(size) # cpp_shm_joint_data1
    ...
    def initialize_shared_memory(self, capacity):
        ...
        self.logger_tc.info("Using C++ shared memory interface for recording.")
        ...
        # Initialize the reader with config
        self.shm_reader = RingBufferReader(self.config)
    ...
    def cleanup_shared_memory_cpp(self):
        ...
        """
        if self.control_loop_language != "cpp" or not hasattr(self, 'shm_reader'):
            return
        ...
        try:
            ...
            self.logger_tc.info("C++ shared memory segment cleaned up.")
        except Exception as e:
            self.logger_tc.error("Failed to clean up C++ shared memory: %s", e)
    ...
    def connect_policy_interface(self):
        ...
        self.policy_interface_process = multiprocessing.Process(
            ...
            args=(self.policy_interface_commup,
                  ...
                  self.shm_joint_data2)  # Shared memory for target positions
        )
    ...
    def _watch_camera_interface_status(self):
        ...
        while self.checking_status:
            # Add debug logging to track if thread is still alive
