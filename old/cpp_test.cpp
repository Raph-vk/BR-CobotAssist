// File: modules/robot/fanuc/cpp/cpp_test.cpp
...
void CppTest::publishSampleToShm(const std::vector<double>& master_position,
...
//--------------------------------------------------------------------------------
void CppTest::publishSampleToPolicyShm(const std::vector<double>& master_position,
                                       const std::vector<double>& send_position_robot,
                                       const std::vector<double>& robot_position,
                                       double robot_position_timestamp,
                                       bool g_on, bool g_off)
...
// Public API
//--------------------------------------------------------------------------------
void CppTest::startControlLoop(bool started_streaming)
{
    started_streaming_ = started_streaming;
    control_thread_ = std::thread(&CppTest::controlLoop, this, started_streaming);
}

void CppTest::stopControlLoop()
{
...
void CppTest::startReceiving()
{
    udp_running_ = true;
    udp_thread_  = std::thread(&CppTest::receiveLoop, this);
}

void CppTest::stopReceiving()
{
    if (udp_running_) {
        udp_running_ = false;
...
void CppTest::setRecording(bool rec)
{
    recording_ = rec;
}
...
// Internal
//--------------------------------------------------------------------------------
void CppTest::controlLoop(bool started_streaming)
{
...
void CppTest::decodeStatusPacket(const uint8_t* data, size_t size)
{
...
// sendJointPos
//--------------------------------------------------------------------------------
bool CppTest::sendJointPos(const std::vector<double>& in_position,
...
// updateRuckigInput
//--------------------------------------------------------------------------------
std::vector<double> CppTest::updateRuckigInput(
...
//--------------------------------------------------------------------------------
bool CppTest::trajectoryCalculation(
    ruckig::Ruckig<0>& otg,
    ruckig::InputParameter<0>& inp,
...
//--------------------------------------------------------------------------------
bool CppTest::handleTrajectoryError(
...
// stopRobot
//--------------------------------------------------------------------------------
bool CppTest::stopRobot(ruckig::Ruckig<0>& otg,
                        ruckig::InputParameter<0>& inp,
                        ruckig::OutputParameter<0>& out)
{
    std::cout << currentTimeFormatted()
...
//--------------------------------------------------------------------------------
void CppTest::determineGripperState(double gripper_state_val)
{
    double now = getEpochTimeSeconds();
...
// roundPosition
//--------------------------------------------------------------------------------
std::vector<double> CppTest::roundPosition(const std::vector<double>& position)
{
...
// J3Interaction
//--------------------------------------------------------------------------------
std::vector<double> CppTest::J3Interaction(const std::vector<double>& action)
{
    auto result = action;
    if (result.size() > 2) {
...
// J3InteractionRev
//--------------------------------------------------------------------------------
std::vector<double> CppTest::J3InteractionRev(const std::vector<double>& action)
{
    auto result = action;
    if (result.size() > 2) {
        // result[2] = result[2] + result[1]
...
//--------------------------------------------------------------------------------
std::vector<double> CppTest::getNextPolicyAction()
{
