...
PYBIND11_MODULE(control_loop_module, m) {
    py::class_<ControlLoop>(m, "ControlLoop")
        ...
        .def("start_control_loop", &ControlLoop::startControlLoop)
        .def("stop_control_loop", &ControlLoop::stopControlLoop,
             py::call_guard<py::gil_scoped_release>())

        // Python -> C++ bindings
        .def("update_target_position", &ControlLoop::updateTargetPosition)
        .def("set_recording", &ControlLoop::setRecording)
        .def("set_policy_active", &ControlLoop::setPolicyActive);
}
